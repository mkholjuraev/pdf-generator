...
const PolicyDetailsTable: React.FC<
  PolicyTableProps & React.HTMLAttributes<HTMLDivElement>
> = ({
  policy,
  nonCompliantSystemsCount,
  totalHostCount,
  compliantSystemCount,
}) => {
  const { testResultHostCount } = policy;
  const compliantPercentage: string =
    testResultHostCount != 0
      ? fixedPercentage(
          Math.floor(100 * (compliantSystemCount / totalHostCount))
        )
      : '0';
  return (
    <Grid>
          ...
          <TableComposable variant="compact">
            <Tbody>
              ...
              <Tr>
                ...
                <Td>{policy?.businessObjective?.title || '--'}</Td>
              </Tr>
            </Tbody>
          </TableComposable>
        </GridItem>
